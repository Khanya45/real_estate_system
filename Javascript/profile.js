...
function getLoggedUser() {
  ...
  if (loggedUser.length > 4) {
    ...
    container.innerHTML = `
        ...
        `;
    fetch(
      "https://desolate-retreat-38151.herokuapp.com/property-by-agent/" +
        parseInt(loggedUser[0]) +
        "/",
      {
        method: "get",
      }
    ...
    container.innerHTML = `
        ...
        `;
    fetch(
      "https://desolate-retreat-38151.herokuapp.com/property-by-user/" +
        loggedUser[0] +
        "/",
      {
        method: "get",
...
function updatedUser() {
  var inputs = document.getElementsByTagName("input");
  let valid = validateForms(inputs[0].value, inputs[1].value, inputs[2].value);
  let validEmail = validateEmail(inputs[1].value);
  if (loggedUser.length == 4) {
    // console.log(loggedUser)
    let updatedUser = {
    ...
    fetch(
      "https://desolate-retreat-38151.herokuapp.com/edit-user/" +
        loggedUser[0] +
        "/",
      {
        method: "PUT",
    ...
    fetch(
      "https://desolate-retreat-38151.herokuapp.com/edit-agent/" +
        loggedUser[0] +
        "/",
      {
        method: "PUT",
...
function deleteUser() {
  if (loggedUser.length > 4) {
    fetch(
      "https://desolate-retreat-38151.herokuapp.com/delete-agent/" +
        loggedUser[0] +
        "/"
    )
      .then((response) => {
    ...
    fetch(
      "https://desolate-retreat-38151.herokuapp.com/delete-user/" +
        loggedUser[0] +
        "/"
    )
      .then((response) => {
...
function updateProperty() {
  ...
  fetch(
    "https://desolate-retreat-38151.herokuapp.com/edit-property/" +
      propertyAgent[0] +
      "/",
    {
      method: "PUT",
...
function deleteProperty(id) {
  if (loggedUser.length > 4) {
    fetch(
      "https://desolate-retreat-38151.herokuapp.com/delete-agent/" + id + "/"
    )
      .then((response) => {
        if (response.status >= 200 && response.status <= 299) {
          return response.json();
    ...
    fetch(
      "https://desolate-retreat-38151.herokuapp.com/delete-user/" + id + "/"
    )
